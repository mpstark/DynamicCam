...
L["<standardSettingsOverridden_desc>"] = "蓝色的类型表示目前被激活的情境覆盖。因此，在覆盖情境激活时，蓝色类型的标准设置不会生效。"
L["Currently overridden by the active situation %s."] = "当前被活动情境覆盖 %s."
L["Situation disabled."] = "情境已禁用。"
L["Situation enabled but condition not fulfilled."] = "情境已启用但条件未满足。"
L["Situation has a script error."] = "情境存在脚本错误。"
L["Help"] = "帮助"
L["Detach"] = "分离"
L["<detach_tooltip>"] = "在一个可自由移动、可调整大小的窗口中打开 DynamicCam 设置。这样在调整设置时不会遮挡屏幕中央。\n\n您也可以随时在聊天中输入 /dc 来打开此窗口。"
L["<detach_combat>"] = "战斗中，我们无法自动关闭游戏的选项面板。\n\n您可以随时在聊天中输入 /dc 来打开分离窗口。"
L["Reattach"] = "重新附加"
L["<reattach_tooltip>"] = "关闭此窗口，并在游戏的选项面板中打开 DynamicCam 设置。"
L["<reattach_combat>"] = "战斗中，我们无法自动打开游戏的选项面板。"
L["Increase opacity"] = "增加不透明度"
L["<opacity_tooltip>"] = "增加此窗口的不透明度以提高可读性。"
L["WARNING"] = "警告"
...
--------------------------------------------------------------------------------
L["Field of View"] = "视野"
L["The camera's field of view. Lower values are more zoomed in, higher values show more of your surroundings."] = "摄像机的视野。较低的值会拉近镜头，较高的值会显示更多周围环境。"
L["<cameraFov_desc>"] = "视野（FOV）决定了屏幕上一次能显示多少游戏世界。较高的视野能显示更多的周围环境，但会使一切看起来稍微远一些；而较低的视野会拉近镜头，带来更具电影感的近距离体验。\n\n使用 DynamicCam 的情境功能，例如可以在跑动时拉宽视野，而在与 NPC 互动或战斗时缩小视野。"
...
L["<selectedSituation_desc>"] = "\n%s颜色代码：%s\n%s- 禁用的情境。%s\n%s- 已启用但目前未激活的情境。%s\n%s- 启用且当前激活的情境。%s\n%s- 启用且条件满足但优先级低于当前激活情境的情境。%s\n%s- 修改过的原始“情境控制”（建议重置）。%s\n%s- 错误的“情境控制”（需要修正）。%s"
...
L["The time in seconds for the transition when ENTERING this situation."] = "进入此情境时过渡所需的时间（秒）。"
L["Exit Transition Time"] = "退出过渡时间"
L["The time in seconds for the transition when EXITING this situation."] = "退出此情境时过渡所需的时间（秒）。"
L["<transitionTime_desc>"] = [[这些过渡时间选项控制情境之间过渡所需的时间。
...
L["<manageProfilesWarning>"] = "与许多插件一样，DynamicCam 使用\"AceDB-3.0\"库来管理配置文件。您需要理解的是，这里没有\"保存配置文件\"这样的功能。您只能创建新的配置文件，并将设置从另一个配置文件复制到当前激活的配置文件中。您对当前激活的配置文件所做的任何更改都会立即保存！没有\"取消\"或\"放弃更改\"这样的功能。\"重置配置文件\"按钮只会重置为全局默认配置文件。\n\n因此，如果您喜欢您的 DynamicCam 设置，您应该创建另一个配置文件，将这些设置复制进去作为备份。当您不将此备份配置文件用作激活的配置文件时，您可以尝试各种设置，并随时通过在\"复制自\"框中选择您的备份配置文件来返回原始配置文件。\n\n如果您想通过宏切换配置文件，可以使用以下命令：\n/run DynamicCam.db:SetProfile(\"配置文件名称\")\n\n"
...
L["Z\no\no\nm"] = "缩\n放"
L["Value"] = "值"
L["Current Zoom/Value:"] = "当前缩放/值:"
L["Left-click: add/drag point\nRight-click: remove point"] = "左键: 添加/拖动点\n右键: 删除点"
L["Copy"] = "复制"
L["<copy_tooltip>"] = "将此曲线复制到剪贴板。然后可以粘贴到任何其他曲线编辑器中，会自动缩放以适应目标值范围。"
L["Paste"] = "粘贴"
L["<paste_tooltip>"] = "用剪贴板中的曲线替换此曲线，缩放以适应此设置的值范围。"
L["<paste_disabled_tooltip>"] = "剪贴板中没有曲线。请先在曲线编辑器中使用复制。"
L["Revert"] = "还原"
L["<revert_tooltip>"] = "将曲线还原到上次保存的状态。"
L["<revert_disabled_tooltip>"] = "没有未保存的更改需要还原。"
L["Save"] = "保存"
L["<save_tooltip>"] = "保存当前曲线。关闭编辑器时未保存的更改将丢失。"
L["<save_disabled_tooltip>"] = "没有未保存的更改。"
L["Close"] = "关闭"
L["<close_tooltip>"] = "关闭编辑器。所有未保存的更改将被丢弃。"
