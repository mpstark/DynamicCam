...
L["<pitch_desc>"] = "如果镜头向上俯仰（较低的\"俯仰\"值），\"俯视缩放\"设置决定了从上方观察角色时这个效果的强度。将\"俯视缩放\"设为0会消除从上方观察时向上俯仰的效果。相反，当你不是从上方观察，或者镜头向下俯仰（较高的\"俯仰\"值）时，\"俯视缩放\"设置几乎没有效果。\n\n因此，你应该先从角色背后观察时找到你喜欢的\"俯仰\"设置。然后，如果你选择了向上俯仰，再从上方观察时找到你喜欢的\"俯视缩放\"设置。\n\n\n当镜头与地面碰撞时，通常会在碰撞点立即执行向上俯仰。另一种选择是镜头在执行俯仰时向角色的脚部靠近。\"智能转轴截止距离\"设置决定了镜头必须在多近的距离内才会执行后者。设为0表示永远不会让镜头靠近（魔兽默认），而设为最大缩放距离39则总是让镜头靠近。\n\n"
...
L["<zoomType_desc>"] = "\n设置：始终将缩放设置为该值。\n\n拉远：仅当镜头当前比此值更近时才设置缩放。\n\n推近：仅当镜头当前比此值更远时才设置缩放。\n\n范围：如果比给定的最大值更远则放大，如果比给定的最小值更近则缩小。如果当前缩放在[最小, 最大]范围内，则不执行任何操作。"
L["Set"] = "设置"
L["Out"] = "拉远"
L["In"] = "推近"
L["Range"] = "范围"
...
L["<hideEntireUI_desc>"] = "\"隐藏\"用户界面和\"仅渐隐\"用户界面是有区别的：渐隐的用户界面元素不透明度为0，但仍然可以与之交互。从 DynamicCam 2.0 开始，如果用户界面元素的不透明度为0，我们会自动隐藏大多数用户界面元素。因此，这个在渐隐后隐藏整个用户界面的选项更像是一个遗留功能。仍然使用它的原因可能是为了避免 DynamicCam 仍未正确隐藏的用户界面元素产生不必要的交互（例如鼠标悬停提示）。\n\n隐藏的用户界面的不透明度当然是0，所以你不能选择不同的不透明度，也不能保持任何用户界面元素可见（除了 FPS 指示器）。\n\n在战斗中，我们无法更改受保护的用户界面元素的隐藏状态。因此，这类元素在战斗中始终设置为\"仅渐隐\"。请注意，小地图\"光点\"的不透明度无法降低。因此，如果你尝试隐藏小地图，\"光点\"在战斗中始终可见。\n\n当你为当前激活的情境勾选此框时，它不会立即生效，因为这也会隐藏此设置框。你必须进入情境才能使其生效，也可以通过上面的情境\"启用\"复选框来实现。\n\n另请注意，隐藏整个用户界面会取消邮箱或 NPC 交互。所以不要将其用于此类情境！"
