...
-- Options
L["Reset"] = "重置"
L["Reset_Desc1"] = "重置为全局默认值："
L["Reset_Desc2"] = "\n（要恢复为特定配置文件的设置，请在“配置文件”标签页中选择该配置文件后操作。）"
L["|cFF00FF00Currently overridden by the active situation \""] = "|cFF00FF00当前设置为情境： \""
L["Override_Standard_Settings"] = "覆盖标准情境"
L["Override_Desc"] = "勾选这个复选框，允许你在激活当前情境时，覆盖标准情境。取消勾选将删除此类型的情境设置。"
L["Custom"] = "自定义： "
L["(modified)"] = "  (已修改)"
L["[Priority"] = " [优先级： "
L["Standard_Settings"] = "标准设置"
L["Standard_Settings_Desc1"] = "当没有任何情境处于激活状态，或者激活的情境没有设置覆盖标准设置的情境设置时，将应用这些标准设置。"
L["Standard_Settings_Desc2"] = "|cFF00FF00绿色的类型表示目前被激活的情境覆盖。因此，在覆盖情境激活时，绿色类型的标准设置不会生效。|r"
L["Standard_Settings_Desc3"] = "当相应的情境激活时，这些情境设置将覆盖标准设置。"
L["Mouse_Zoom"] = "鼠标缩放"
L["Maximum_Camera_Distance"] = "最大镜头距离"
L["Maximum_Camera_Distance_Desc"] = "镜头镜头可以从你的角色拉远多少码的距离。\n|cff909090cvar: cameraDistanceMaxZoomFactor|r"
L["Camera_Zoom_Speed"] = "镜头缩放速度"
L["Camera_Zoom_Speed_Desc"] = "镜头镜头缩放的速度。\n|cff909090cvar: cameraZoomSpeed|r"
L["Zoom_Increments"] = "镜头缩放增量"
L["Zoom_Increments_Desc"] = "每次鼠标滚轮镜头应该移动多少码。"
L["Use_Reactive_Zoom"] = "使用响应缩放"
L["Quick-Zoom_Additional_Increments"] = "快速缩放额外增量"
L["Quick-Zoom_Additional_Increments_Desc"] = "当快速缩放时，每次鼠标滚轮滚动应该增加多少码。"
L["Quick-Zoom_Enter_Threshold"] = "快速缩放阈值"
L["Quick-Zoom_Enter_Threshold_Desc"] = "\"响应缩放目标\"与\"实际缩放值\"之间至少需要多少码的距离，才能触发快速缩放功能。"
L["Maximum_Zoom_Time"] = "最大缩放时间"
L["Maximum_Zoom_Time_Desc"] = "镜头会在这个时间内将\"实际缩放值\"调整到\"响应缩放目标\"。"
L["Help"] = "帮助"
L["Toggle_Visual_Aid"] = "视觉辅助开关"
L["ReactiveZoomDescription"] = "使用 DynamicCam 的响应缩放功能，鼠标滚轮控制\"响应缩放目标\"。每当\"响应缩放目标\"与\"实际缩放值\"不同时，DynamicCam 会改变\"实际缩放值\"，直到他再次与\"响应缩放目标\"相同。\n\n这种缩放变化的速度取决于\"镜头缩放速度\"和 \"最大缩放时间\"。如果\"最大缩放时间\"设置的比较短，无论\"镜头缩放速度\"如何设置，缩放总会很快执行。要实现舒缓的缩放变化，你必须将\"最大缩放时间\"设置得更长，同时把\"镜头缩放速度\"设置为较低的值。\n\n为了实现随着鼠标滚轮快速滚动更快地缩放镜头，请使用\"快速缩放功能\"：如果\"响应缩放目标\"与\"实际缩放值\"的偏差超过了\"快速缩放阈值\"，每次鼠标滚轮的滚动都会增加\"快速缩放额外增量\"。\n\n为了感受这些功能是如何工作的，你可以在寻找合适设置的同时，开启视觉辅助。你也可以通过左键拖弋操作来自动移动这个图表。右键点击可以关闭它。"
L["Enhanced_minimal_zoom-in"] = "强化最小视角"
L["Enhanced_minimal_zoom-in_desc"] = "响应缩放允许你把镜头放大到比最近还近。你可以通过在第一人称视角时再次滚动鼠标实现这一点。\n\n启用\"强化最小视角\"后，我们会强制镜头在放大时也停留在这个视角上，而不是立即切换回第一人称视角。你也可以把此理解为\"狙击模式\"。\n\n|cFFFF0000启用\"强化最小视角\"可能会在CPU受限的情境下导致帧率下降15%。|r"
L["Reload_UI"] = "|cFFFF0000需要使用 /reload 重载界面！|r"
L["Mouse_Look"] = "鼠标视角"
L["Horizontal_Speed"] = "水平速度"
L["Horizontal_Speed_desc"] = "在鼠标视角模式下，镜头的水平移动速度是多少。\n|cff909090cvar: cameraYawMoveSpeed|r"
L["Vertical_Speed"] = "垂直速度"
L["Vertical_Speed_desc"] = "鼠标视角模式下，镜头的垂直移动速度是多少。\n|cff909090cvar: cameraPitchMoveSpeed|r"
L["MouseLookDescription"] = "当你在\"鼠标视角\"模式下移动鼠标时，镜头移动的幅度。\n\n在《魔兽世界》的默认界面设置中，\"鼠标视角移动速度\"滑块同时控制水平和垂直速度，并且自动将水平速度设置为垂直速度的2倍。DynamicCam 覆盖了这一设置，允许你进行更加个性化的设置。"
L["Horizontal_Offset"] = "水平偏移"
L["Camera_Over_Shoulder_Offset"] = "越肩视角偏移"
L["Camera_Over_Shoulder_Offset_desc"] = "调整镜头在角色左侧或者右侧的位置。\n|cff909090cvar: test_cameraOverShoulder|r\n\n要使这个设置生效，DynamicCam 会自动临时禁用游戏中的晕动症设置。因此，如果你需要使用晕动症设置，请不要使用水平偏移。\n\n当你选中自己的角色时，《魔兽世界》会自动切换到偏移量为0。我们无法对此进行更改。我们也无法解决镜头和墙壁发生碰撞时可能出现的偏移抖动问题。一个可能的解决方法是在室内时使用很小或不使用偏移。\n\n此外，《魔兽世界》会根据玩家模型或坐骑不同，奇怪地应用不同的偏移量。如果你希望有一个恒定的偏移量，Ludius（插件开发者）正在开发另一个插件 CameraOverShoulder 来解决这个问题。"
L["ShoulderOffsetZoom"] = "根据缩放调整越肩偏移"
L["Enable"] = "启用"
L["ShoulderOffsetZoomReset_Desc1"] = "重置为全局默认值："
L["ShoulderOffsetZoomReset_Desc2"] = " 和 "
L["ShoulderOffsetZoomReset_Desc3"] = "\n（要恢复特定配置文件的设置，请在“配置文件”标签中恢复该配置文件。）"
L["No_offset_when_below_this_zoom_level"] = "当缩放级别低于此值时无偏移："
L["No_offset_when_above_this_zoom_level_Desc"] = "当镜头比此缩放级别更近时，偏移量已达到零。"
L["Real_offset_when_above_this_zoom_level"] = "当缩放级别高于此值时的真实偏移："
L["Real_offset_when_above_this_zoom_level_Desc"] = "当镜头比此缩放级别更远时，偏移量已达到其设定值。"
L["ShoulderOffsetZoomDescription"] = "在缩放时使肩部偏移量逐渐过渡到零。两个滑块定义了此过渡发生的缩放级别范围。此设置是全局的，不特定于情境。"
L["Vertical_Pitch"] = "垂直俯仰"
L["Pitch(on_ground)"] = "俯仰（在地面上）"
L["Pitch_(flying)"] = "俯仰（飞行中）"
L["Down_Scale"] = "向下缩放"
L["Smart_Pivot_Cutoff_Distance"] = "智能转动停止距离"
L["PitchDescription"] = "如果镜头向上俯仰（“俯仰”值较低），“向下缩放”设置决定了在从上方看角色时这种效果的影响程度。将“向下缩放”设置为0可以抵消从上方看时向上俯仰的效果。相反，当你不是从上方看，或者如果镜头向下俯仰（“俯仰”值较大）时，“向下缩放”设置几乎没有效果。\n\n因此，你应该首先找到从背后看你角色时喜欢的“俯仰”设置。之后，如果你选择了向上俯仰，从上方看时找到你喜欢“向下缩放”设置。\n\n当镜头与地面碰撞时，它通常会在镜头到地面碰撞点进行向上俯仰。另一种选择是镜头在执行这种俯仰时更靠近角色的脚。“智能转动截止距离”设置决定了镜头必须在多远的距离内才能执行后者。将其设置为0则从不移动镜头（WoW的默认设置），而将其设置为最大缩放距离39则总是移动镜头。\n\n"
L["Target_Focus"] = "目标焦点"
L["Enemy_Target"] = "敌方目标"
L["Horizontal_Strength"] = "水平强度"
L["Vertical_Strength"] = "垂直强度"
L["Interaction_Target_(NPCs)"] = "交互目标（NPC）"
L["TargetFocusDescription"] = "如果启用，镜头会自动尝试将目标拉近屏幕中心。强度决定了这种效果的强度。\n\n如果“敌方目标焦点”和“交互目标焦点”都启用，后者似乎有一个奇怪的错误：当首次与NPC交互时，镜头会像预期的那样平滑移动到新角度。但是当你退出交互时，它会立即跳转到之前的角度。然后当你再次开始交互时，它再次跳转到新角度。这在与新NPC交谈时是可重复的：只有第一次过渡是平滑的，所有后续的都是立即的。\n如果你想要同时使用“敌方目标焦点”和“交互目标焦点”，一个变通方法是只在需要它且不太可能发生NPC交互的DynamicCam情境下激活“敌方目标焦点”（比如战斗）。"
L["Head_Tracking"] = "头部追踪"
L["HeadTrackingEnable_Desc"] = "|cff909090cvar: test_cameraHeadMovement\n强度\n\n这也可以作为一个0到1之间的连续值，但它只是分别乘以站立强度和移动强度。所以真的不需要另一个滑块。|r"
L["Strength_(standing)"] = "强度（站立）"
L["Inertia_(standing)"] = "惯性（站立）"
L["Strength_(moving)"] = "强度（移动）"
L["Inertia_(moving)"] = "惯性（移动）"
L["Inertia_(first_person)"] = "惯性（第一人称）"
L["Range_Scale"] = "范围缩放"
L["Range_Scale_Desc"] = "超过此镜头距离时减少或禁用头部追踪。（见下文解释。）\n|cff909090cvar: test_cameraHeadMovement\nRangeScale（滑块值转换）|r"
L["Dead_Zone"] = "死区"
L["Dead_Zone_Desc"] = "头部移动不影响镜头的半径。（见下文解释。）\n|cff909090cvar: test_cameraHeadMovement\nDeadZone（滑块值 / 10）|r\n|cffe00000需要/reload才能生效！|r"
L["HeadTrackingDescription"] = "启用头部追踪后，镜头会跟随角色头部的移动。（虽然这可能有助于沉浸感，但如果你对晕动症敏感，也可能导致恶心。）\n\n“强度”设置决定了这种效果的强度。将其设置为0可以禁用头部追踪。“惯性”设置决定了镜头对头部移动的反应速度。将其设置为0也可以禁用头部追踪。“站立”、“移动”和“第一人称”三种情境可以单独设置。“第一人称”没有“强度”设置，因为它分别假设“站立”和“移动”的“强度”设置。如果你想单独启用或禁用“第一人称”，请使用“惯性”滑块来禁用不需要的情境。\n\n“范围缩放”设置可以设置超过此镜头距离时减少或禁用头部追踪。例如，将滑块设置为30，当镜头距离角色超过30码时，将没有头部追踪。但是，从完全头部追踪到没有头部追踪有一个逐渐过渡，从滑块值的三分之一开始。例如，将滑块值设置为30，当镜头距离小于10码时，有完全头部追踪。超过10码时，头部追踪逐渐减少，直到在30码外完全消失。因此，滑块的最大值是117，允许在最大镜头距离39码时有完全头部追踪。（提示：使用我们的“鼠标缩放”视觉辅助工具在设置时检查当前镜头距离。）\n\n“死区”设置可以用来忽略较小的头部移动。将其设置为0可以让镜头跟随每一个微小的头部移动，而将其设置为更大的值则只跟随较大的移动。请注意，更改此设置只有在重新加载UI（在控制台中输入/reload）后才生效。"
L["Situations"] = "情境"
L["Select_a_situation_to_setup"] = "选择一个情境来设置"
...
L["SelectedSituation_Desc1"] = "如果选中此框，DynamicCam将在其条件满足且没有其他优先级更高的情境激活时进入情境 \""
L["SelectedSituation_Desc2"] = "\"。"
L["DeleteCustom_Desc1"] = "删除自定义情境 \"" 
L["DeleteCustom_Desc2"] = "\"。\n（不会有'你确定吗？'的提示！）"
L["Create a new custom situation."] = "创建一个新的自定义情境。"
L["Situation_Actions"] = "情境指令"
...
L["CameraSommthNote_Name"] = [[|cFFEE0000注意：|r 您正在使用WoW的“镜头跟随风格”，它会自动将镜头放置在玩家后面。这在您处于自定义视角时不起作用。您可以在不需要镜头跟随的情境中使用自定义保存视角（例如，NPC互动）。但在退出情境后，您必须返回到非自定义的默认视角，以便再次使镜头跟随工作。]]
...
视角4：缩放13.8，俯仰30
视角5：缩放13.8，俯仰10]]
L["ReturnString"] = "|cFFEE0000警告：|r 您正在使用相同的视角作为保存视角和恢复为默认视角。用作恢复为默认视角将会重置它。只有在您确实想将其用作非自定义保存视角时才这样做。\n"
...
L["Always"] = "总是"
L["Adaptive"] = "自适应"
L["ZoomDescription_Name"] = [[要确定当前的缩放级别，您可以使用“视觉辅助”（在DynamicCam的“鼠标缩放”设置中切换）或使用控制台命令：
...
L["If you set a time greater than 0 here, the continuous rotation will not immediately start at its full rotation speed but will take that amount of time to accelerate. (Only noticeable for relatively high rotation speeds.)"] = "如果您在这里设置的时间大于0，持续转动不会立即以全速开始，而是会花费这段时间来加速。（只有在相对较高的转动速度下才会明显感知。）"
L["How long it should take to assume the new camera angle. If a too small value is given here, the camera might rotate too far, because we only check once per rendered frame if the desired angle is reached.\n\nIf a situation assigns the variable \"this.rotationTime\" in its on-enter script (see \"Situation Controls\"), the setting here is overriden. This is done e.g. in the \"Hearth/Teleport\" situation to allow a rotation time for the duration of the spell cast."] = "需要多长时间来调整到新的摄像机角度。如果这里给出的值太小，摄像机可能会转动过头，因为我们每渲染一帧时只检查一次是否达到了期望的角度。\n\n如果某个情境在其进入脚本中分配了变量“this.rotationTime”（参见“情境控制”），这里的设置将被覆盖。例如，在“炉石/传送”情境中这样做，以便为施法时间内塞入一个转动时间。"
...
L["Do not fade out the Minimap.\n\nNotice that we cannot reduce the opacity of the \"blips\" on the Minimap. These can only be hidden together with the whole Minimap, when the UI is faded to 0 opacity."] = "不要渐隐小地图。\n\n请注意，我们不能减少小地图上“光点”的不透明度。当用户界面渐隐到0不透明度时，这些只能与整个小地图一起隐藏。"
...
L["HeadTrackingDescription_Name"] = [[有时你可能希望在常态隐藏界面的情况下显示界面。在旧版本的 DynamicCam 中有个规则，允许你在按下ESC键时显示界面。但这样做有个缺点，当ESC键用于其他作用，比如关闭窗口、取消施法等时，界面也会显示。取消勾选上面的复选框可以禁用此功能。
...
L["EventsDescription_Name"] = [[在这里，你可以定义所有游戏中的事件，DynamicCam应该检查这些事件的条件，以确定是否进入或退出当前情境。
...
L["InitialisationDescription_Name"] = [[一个情境的初始化脚本在 DynamicCam 加载时（以及情境被修改时）运行一次。你通常会在其中放入你希望在其他任何脚本（条件、进入、退出）中应用的内容。这可以使这些脚本更短一些。
...
L["ConditionDescription_Name"] = [[条件脚本在每次触发此情境的游戏内事件时运行。如果且仅当此情境应该处于活动状态时，脚本应返回“true”。
...
L["ExecuteOnEnterDescription_Name"] = [[情境带有的进入时脚本在每次进入该情境时执行。
...
L["ExecuteOnExitDescription_Name"] = [[情境带有的退出时脚本在每次退出该情境时运行。到目前为止，还没有情境使用这个功能。
...
L["About"] = "关于"
L["|cFFEE0000WARNING!|r"] = "|cFFEE0000警告！|r"
...
L["|cFFFF0000Attention|r"] = "|cFFFF0000注意|r"
L["The \""] = "这个\""
...
L["|cFFFF0000While you are using horizontal camera offset, DynamicCam prevents CameraKeepCharacterCentered!|r"] = "|cFFFF0000当你使用水平镜头偏移时，DynamicCam会阻止CameraKeepCharacterCentered起效！|r"
L["|cFFFF0000While you are using vertical camera pitch, DynamicCam prevents CameraKeepCharacterCentered!|r"] = "|cFFFF0000当你使用垂直镜头俯仰时，DynamicCam会阻止CameraKeepCharacterCentered起效！|r"
L["|cFFFF0000While you are using horizontal camera offset, DynamicCam prevents CameraReduceUnexpectedMovement!|r"] = "|cFFFF0000当你使用水平镜头偏移时，DynamicCam会阻止CameraReduceUnexpectedMovement起效！|r"
L["|cFFFF0000cameraView ="] = "|cFFFF0000镜头视角 ="
L["prevented by DynamicCam!|r"] = "被DynamicCam阻止！|r"
